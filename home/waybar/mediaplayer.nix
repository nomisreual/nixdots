# { pkgs, ... }:
{ pkgs ? import <nixpkgs> { } }:
pkgs.writers.writePython3Bin "mediaplayer"
{
  libraries = [
  ...
  makeWrapperArgs = [
    ...
    "PATH"
    ":"
    "${pkgs.lib.makeBinPath [ pkgs.playerctl pkgs.gobject-introspection ]}"

    "--set"
    "GI_TYPELIB_PATH"
    "${pkgs.lib.makeLibraryPath [ pkgs.playerctl pkgs.gobject-introspection ]}"
  ];
} ''
  import gi
